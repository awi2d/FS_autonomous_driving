...
SLAM::SLAM(bool clustering, bool frontend, bool negativeinformation){
    ...
    //typedef g2o::LinearSolverEigen<g2o::BlockSolver<g2o::BlockSolverTraits<-1, -1> >::PoseMatrixType> SlamLinearSolver;
    ...
    this->gnsspos_information <<
        this->gnss_information(0, 0), 0, 0, 0, 0,
        0, this->gnss_information(1, 1), 0, 0, 0,
        0, 0, 1/(0.01*0.01), 0, 0, // no heading -> car is not moving -> velocity and yawrate are 0.
        0, 0, 0, 1/(0.01*0.01), 0,
        0, 0, 0, 0, 1/(0.01*0.01);
    this->odometry_information << //smoothing for 5 secodns, cause otherwise they are unreasonable large
        1.0/0.0181, 0, 0, 0, 0, 0, // error in estimated_posn (from previus carpose) - true_posn. Should be 0, as velocity and yawrate should change until the positions match.
        0, 1.0/0.0181, 0, 0, 0, 0, // error in estimated_pose (from previus carpose) - true_pose
        0, 0, 1.0/0.0279, 0, 0, 0, // error in estimated_heading (from previus carpose) - true_heading
        0, 0, 0, 1.0/0.136, 0, 0, // error in (previus_vx-current_vx)=0
        0, 0, 0, 0, 1.0/(0.01*0.01), 0, // error in (previus_vy-current_vy)=0  // no real data for vy available, so this is just a guess
    ...
    this->borangec = new custom_g2otypes::Vertex_conepos(2); //2 = color orange
    ...
    this->yorangec = new custom_g2otypes::Vertex_conepos(2);
...
void SLAM::add_vpdetections(second time, int camL3_frnr, std::vector<std::tuple<int, distheading>> vp_det) {
        ...
        for (auto &i: vp_det) {
            ...
            if (color == 0 || color == 1) {
                ...
                if (color == 2) {
                    ...
                    //TODO maybe set estimate of orange cone?
                }else{
                    if (color == 3) {
            ...
            m_position rel_mpos_unrotated =
                    distazimuth_to_meter(std::get<0>(dist_heading), std::get<1>(dist_heading));
...
void SLAM::add_GNSS_mes(pose_ext gnss_measurement, int camL3_frnr){
    ...
    if(current_carpose != nullptr){
        ...
        if(camL3_frnr > 1700){
            ...
            //gnss_constraint->setInformation(this->gnss_information*0.01);
            //this->optimizer.addEdge(gnss_constraint);
        }
...
void SLAM::optimise(){
    ...
    //very that graph is able to be optimised:
    ...
    if(this->do_clustering){
        ...
        for (int i = 0; i < conevertexe.size(); i++) {
            ...
            if (!i_in_cluster) {
                ...
                int color = this->get_vertexconepos(iid)->color;
